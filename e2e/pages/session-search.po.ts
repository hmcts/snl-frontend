...
export class SessionSearchPage {
    ...
    private async selectCheckBoxInRowWithValues(table: Table, paginator: Paginator, ...values: string[]) {
        await browser.wait(ExpectedConditions.visibilityOf(await table.rowThatContainsAtAnyPage(paginator, ...values)),
            Wait.normal, `Row with values: ${values} is not visible`);
        const row = await table.rowThatContainsAtAnyPage(paginator, ...values);
