...
export class TransactionDialogPage {
    ...
    private sessionButtonSelector = this.parentElement.element(
        by.cssContainingText('.mat-button-wrapper', 'Sessions')
    )
    ...
    isSessionCreationSummaryDisplayed(): promise.Promise<boolean> {
        browser.wait(ExpectedConditions.visibilityOf(this.summaryCreationElement), Wait.normal)
        return this.summaryCreationElement.isDisplayed()
    }

    async isProblemWithTextDisplayed(problemText: string): Promise<boolean> {
        browser.wait(ExpectedConditions.visibilityOf(this.problemsDiv), Wait.normal)
